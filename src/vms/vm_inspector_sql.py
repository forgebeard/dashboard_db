...
from core.constants import VM_STATUS_MAP, IMAGE_STATUS_MAP
    ...
    try:
        with InspectorBase(db_name) as insp:
            ...
            vm = insp.fetch_one(
                ...
                WHERE v.vm_guid::text = :vm_search LIMIT 1
                """,
                {"vm_search": vm_search},
            )
            ...
            total_vcpu = (vm["num_of_sockets"] or 0) * (vm["cpu_per_socket"] or 0) * (
            ...
            disks = insp.fetch_all(
                ...
                SELECT bd.disk_alias, bd.disk_id, i.image_guid, i.size, i.imagestatus,
                       ...
                       i.creation_date
                FROM base_disks bd
                JOIN vm_device vd ON bd.disk_id = vd.device_id AND vd.vm_id = :vm_guid
            ...
            if not nics:
                ...
                ips = insp.fetch_all(
                    ...
                    FROM vm_guest_agent_interfaces WHERE vm_id = :vm_guid
                    """,
                    {"vm_guid": vm["vm_guid"]},
                )
                if ips:
            ...
            issues = []
            locked_row = insp.fetch_one(
                """
                SELECT COUNT(*) AS locked_count FROM images i
                JOIN vm_device vd ON i.image_group_id = vd.device_id
                WHERE vd.vm_id = :vm_guid AND i.imagestatus IN (:locked_code, :illegal_code)
                """,
                {
                    "vm_guid": vm["vm_guid"],
            ...
            locked_count = locked_row["locked_count"] if locked_row else 0
            if locked_count > 0:
